...
public class OrderTrackingTransformer {
	...
	if (null != order) {
		...
		for (final DeliveryGroup deliveryGroup : attribute.getDeliveryGroups()) {
		    ...
		    final String currentStatus = deliveryGroup.getCurrentStatus();
			...
			if (("DELIVERED".equals(lifeCycle.getStatus()) || "COLLECTED".equals(lifeCycle.getStatus()))
			...
			if (lifeCycle.isCompleted() && (lifeCycle.getStatus().equals("Order On the way")
			    ...
			    lifeCycle.setTrackingUrl("https://www.parcelforce.com/track-trace?trackNumber="
				    + lifeCycle.getRefernceNumber());
			...
			deliveryGroup.setDeliveryGroupMsg(messageSource.getMessage(
				...
				Locale.ENGLISH));
		    } else if ("DELIVERED".equals(currentStatus) || "COLLECTED".equals(currentStatus)) {
			String messageKey = null;
			if (deliveryDate != null && DateTimeComparator.getDateOnlyInstance()
				.compare(deliveryGroup.getDeliveryDate(), deliveryDate) > 0) {
			    messageKey = "earlyDeliveredGroupMsg";
			} else {
			    messageKey = "deliveredGroupMsg";
			}
			if ("CC".equals(fulfilmentSourceType)) {
			    messageKey = messageKey + "CC";
			}

			deliveryGroup.setDeliveryGroupMsg(messageSource.getMessage(messageKey,
				new Object[] {
					new SimpleDateFormat(globalConfiguration.getDeliveryGroupHeaderDateFormat())
						.format(deliveryGroup.getDeliveryDate()) },
				Locale.ENGLISH));
